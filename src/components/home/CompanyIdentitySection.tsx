...
export default function CompanyIdentitySection({ locale }: { locale: string }) {
  ...
  const facts = isKo
    ...
    : [
        ...
        ['Based in', 'Gangseo-gu, Busan, Republic of Korea'],
        ['Focus', 'Online water measurement and field monitoring'],
      ]

  return (
    <section className="border-t border-[#d2d7d4] bg-[#f5f6f4] py-20 lg:py-28">
      <div className="container-custom grid gap-12 lg:grid-cols-[0.82fr_1.18fr] lg:gap-20">
        <div>
          ...
          <h2 className="mt-5 max-w-xl break-keep text-3xl font-bold leading-[1.25] tracking-normal text-[#151a19] lg:text-4xl">
            {isKo
              ? '부산 기반 수질계측기 개발·제조 기업, 워터비'
              : 'Waterbee, a water-quality instrument company based in Busan'}
          </h2>
          <p className="mt-6 max-w-xl break-keep text-sm leading-7 text-[#68716f] lg:text-base">
            {isKo
        ...
        <div>
          <dl className="border-t border-[#9fa8a5]">
            {facts.map(([label, value]) => (
              <div key={label} className="grid gap-2 border-b border-[#d2d7d4] py-5 sm:grid-cols-[150px_1fr] sm:gap-8">
